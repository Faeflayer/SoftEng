...
```
### Результат.
![Меню]( https://github.com/Faeflayer/SoftEng/blob/Tema_6/pics/self1.png)
...
```
### Результат.
![Меню](https://github.com/Faeflayer/SoftEng/blob/Tema_6/pics/self2.png )
...
`
Кортеж - это неизменяемая структура данных, которая может содержать элементы любого типа. 
Элементы кортежа доступны по индексу, начиная с 0, и могут быть любых типов данных.
Для создания кортежа нужно использовать круглые скобки - example = (1,2,3)
В кортеже можно получить доступ к значению с помощью индекса -  example[0]

Словарь - это изменяемая структура данных, представляющая собой набор пар ключ-значение, ключи должны быть уникальны и неизменяемы.
Создается словарь с использование фигурных скобок и указывая пары ключ-значение - example - {"room": "111", password: "1111"}
В словаре с помощью ключа можно получить значение - example["room"]
`
